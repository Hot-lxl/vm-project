...
import CartContrl from "@/components/CartControl/CartControl.vue";
import { mapState, mapGetters } from "vuex";
export default {
  name: "ShopCart",
  data() {
    return {
      isShow: false, //控制购物车列表显示
    };
  },
  components: {
    CartContrl,
  ...
  computed: {
    ...
    listShow() {
      ...
      // 为true说明列表已经展开
      if (this.isShow) {
        // 状态发生了改变但是不能滚动所以将延迟回调到下一次
        this.$nextTick(() => {
          // 判断没有scroll才创建滚动 实现Bsroll的实例是一个单例子
          if (!this.scroll) {
            // 为商品列表添加滚动
            ...
            // 有scroll实例，让滚动条刷新一下，重新统计内容的高度（以前的bug）
            this.scroll.refresh();
          }
        });
      }
