...
export default {
  ...
  data() {
    return {
      supportsClasses: ["activity-green", "activity-orange", "activity-red "], //用于存储颜色的class名字
    };
  },
  mounted() {
    //判断info的pics图片是否存在
  ...
  methods: {
    ...
    _initScollr() {
      ...
      // 修改ul的宽度
      ul.style.width = (ilWidth + space) * count - space + "px";
      // 创建商家实景图的水平滑动
  ...
  watch: {
    // 监听info的改变，一刷新info就不存在了
    info() {
      //info数据更新了但是页面还没有真正的更新
      this.$nextTick(() => {
        this._initScollr();
      });
    },
